...
def run() -> object:
    ...
    suite.check_equal("factory default theme is dark", factory.general.theme, "Dark")
    suite.check_equal("factory default camera index is zero", factory.camera.index, 0)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        suite.check_equal("camera override is applied", config.camera.index, 2)
        suite.check_false("keyboard show_selfie override is applied", config.keyboard.show_selfie)
        suite.check_close("mouse sensitivity override is applied", config.mouse_motion.sensitivity, 1.23)
