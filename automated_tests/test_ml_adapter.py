...
def run() -> object:
    ...
    suite.check_equal("unavailable prediction reports the reason", unavailable.status, "missing model")
    ...
    suite.check_equal("confirmed undo emits one Ctrl+Z hotkey", undo_update.actions, (Hotkey(keys=("ctrl", "z")),))
    undo_repeat = adapter.update(prediction("undo"), state, now=1.2)
    suite.check_equal("held undo does not repeat immediately", undo_repeat.actions, ())

    adapter = MLControlAdapter(config)
    state = RuntimeState(mode=Mode.MOUSE)
    adapter.update(prediction("hold"), state, now=1.0)
    hold_update = adapter.update(prediction("hold"), state, now=1.1)
    suite.check_true("confirmed hold activates clutch in mouse mode", hold_update.hold_active)

    adapter = MLControlAdapter(config)
    state = RuntimeState(mode=Mode.KEYBOARD)
    adapter.update(prediction("hold"), state, now=1.0)
    hold_keyboard = adapter.update(prediction("hold"), state, now=1.1)
    suite.check_false("hold does not stay active in keyboard mode", hold_keyboard.hold_active)
    ...
    suite.check_equal("toggle is armed before hold time completes", state.control_enabled, True)
    toggled = adapter.update(prediction("toggle"), state, now=1.5)
    suite.check_false("toggle hold flips control_enabled once threshold is met", state.control_enabled)
    suite.check_true("toggle status mentions toggled", "toggled" in toggled.status)
    ...
    suite.check_false("toggle cooldown blocks immediate second flip", state.control_enabled)
    suite.check_true("cooldown status stays on toggle path", cooldown_update.status.startswith("toggle"))
    ...
    suite.check_true("toggle can fire again after cooldown window", state.control_enabled)
